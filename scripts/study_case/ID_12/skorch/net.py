...
class NeuralNet:
    ...
    def __init__(
        ...
        self.scheduler = TorchScheduler(name="skorch")
        '''inserted code'''
    ...
    def train_step_single(self, Xi, yi, **fit_params):
        ...
        **fit_params : dict
        ...
        # print(f"Iteration:{self.ptr} Time cost:{datetime.now() - self.s1}")
        # self.ptr += 1
        # if torch.isnan(loss).any() or torch.isinf(loss).any():
        #     print(f"ERROR: NaN found! Time cost:{datetime.now() - self.s1}")
        #     sys.exit(0)

        '''inserted code'''
        self.scheduler.loss_checker(loss)
        '''inserted code'''

        loss.backward()

        '''inserted code'''
        self.scheduler.check_time()
        '''inserted code'''
