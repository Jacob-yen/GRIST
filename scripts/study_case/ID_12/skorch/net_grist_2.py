...
from scripts.study_case.ID_12.skorch.dataset import get_len
from scripts.study_case.ID_12.skorch.dataset import unpack_data
from scripts.study_case.ID_12.skorch.dataset import uses_placeholder_y
...
from scripts.study_case.ID_12.skorch.history import History
from scripts.study_case.ID_12.skorch.setter import optimizer_setter
from scripts.study_case.ID_12.skorch.utils import FirstStepAccumulator
from scripts.study_case.ID_12.skorch.utils import TeeGenerator
from scripts.study_case.ID_12.skorch.utils import duplicate_items
from scripts.study_case.ID_12.skorch.utils import get_map_location
...
class NeuralNetFGSM:
    ...
    def __init__(
        ...
        self.gradient_search = GradientSearcher(name="skorch_grist",switch_data=True)
        """inserted code"""
    ...
    def train_step_single(self, Xi, yi, **fit_params):
        ...
        **fit_params : dict
        ...
        """insert code"""
        # torch.log/div/log/reciprocal/rsqrt
        obj_function = torch.min(torch.abs(y_pred))

        obj_grads = torch.autograd.grad(obj_function, Xi, retain_graph=True)[0]
        """insert code"""
        ...
        Xi, scores_rank = self.gradient_search.update_batch_data(monitor_var=monitor_vars, input_data=Xi)
        """inserted code"""
        ...
        '''inserted code'''
        self.gradient_search.check_time()
        '''inserted code'''
    ...
    def fit_loop(self, X, y=None, epochs=None, **fit_params):
        ...
        **fit_params : dict
        ...
        "inserted code"
        dataloader_iter = iter(self.get_iterator(dataset_train, training=True))
        data = next(dataloader_iter)
        Xi_train, yi_train = unpack_data(data)
        Xi_train = torch.autograd.Variable(Xi_train, requires_grad=True)
        max_val, min_val = Xi_train.max().item(), Xi_train.min().item()
        self.gradient_search.build(batch_size=self.batch_size, min_val=min_val, max_val=max_val)
        ...
        while True:
            ...
            train_batch_count = 0
            for _ in self.get_iterator(dataset_train, training=True):
                # Xi, yi = unpack_data(data)
                yi_res_train = yi_train if not y_train_is_ph else None
                self.notify('on_batch_begin', X=Xi_train, y=yi_res_train, training=True)

                step = self.train_step(Xi_train, yi_train, **fit_params)
                Xi_train,scores_rank = step['Xi'],step['scores_rank']
                """inserted code"""
                try:
                    data = next(dataloader_iter)
                    new_batch_xs, new_batch_ys = unpack_data(data)
                ...
                old_data_dict = {'x': Xi_train, 'y': yi_train}
                Xi_train, yi_train = self.gradient_search.switch_new_data(new_data_dict=new_data_dict,
                ...
                self.history.record_batch('train_batch_size', get_len(Xi_train))
                # self.notify('on_batch_end', X=Xi_train, y=yi_res_train, training=True, **step)
                train_batch_count += 1
