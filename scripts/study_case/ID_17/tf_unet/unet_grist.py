...
from scripts.study_case.ID_17.tf_unet.layers import (weight_variable, weight_variable_devonc, bias_variable,
...
class Unet(object):
    ...
    def _get_cost(self, logits, cost_name, cost_kwargs):
        ...
        if cost_name == "cross_entropy":
            ...
            """insert code"""
            # exp/expm1
            self.obj_function = -1* tf.reduce_max(logits)
            self.obj_grads = tf.gradients(self.obj_function, self.x)[0]
            print("there")
            print(self.obj_grads)
...
class Trainer(object):
    ...
    def __init__(self, net, batch_size=1, optimizer="momentum", opt_kwargs={}):
        ...
        self.gradient_search = GradientSearcher("tf_unet_grist")
        """inserted code"""
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1, restore=False):
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                # batch_x, batch_y = data_provider(self.batch_size)
                """inserted code"""
                monitor_vars = {'loss': self.net.cost, 'obj_function': self.net.obj_function, 'obj_grad': self.net.obj_grads}
                feed_dict = {self.net.x: batch_x,
                             self.net.y: util.crop_to_shape(batch_y, pred_shape),
                             self.net.keep_prob: dropout}

                batch_x, scores_rank = self.gradient_search.update_batch_data(session=sess, monitor_var=monitor_vars,
                ...
                _, loss, lr, gradients = sess.run((self.optimizer, self.net.cost, self.learning_rate_node, self.net.gradients_node),
                                                  feed_dict={self.net.x: batch_x,
                                                             ...
                                                             self.net.keep_prob: dropout})
                """inserted code"""
                new_batch_x, new_batch_y = data_provider(self.batch_size)
                new_data_dict = {'x': new_batch_x, 'y': new_batch_y}
                old_data_dict = {'x': batch_x, 'y': batch_y}
                batch_x, batch_y = self.gradient_search.switch_new_data(new_data_dict=new_data_dict,
