...
def unsafe_softmax(logits):
    ...
    return tf.exp(tf.clip_by_value(logits, -87, 87)) / tf.reduce_sum(
        # tf.exp(tf.clip_by_value(logits, -100, 87)), axis=1, keepdims=True
...
def unsafe_cross_entropy(probabilities, labels):
    ...
    return -tf.reduce_sum(labels * tf.log(tf.clip_by_value(probabilities, 1e-6, 1)), axis=1)
...
def main(_):
    ...
    gradient_search = GradientSearcher(name="TensorFuzz_truediv_grist")
