...
import scripts.study_case.ID_2.model as model
...
class LogisticRegression(model.Model):
    ...
    def _train_model(self, train_set, train_labels, validation_set, validation_labels):
        ...
        scheduler = TensorFlowScheduler(name="ghips6")
        '''inserted code'''
        ...
        while True:
            ...
            for idx,batch in enumerate(batches):
                ...
                cost_val,_ = self.tf_session.run([self.cost,self.train_step], feed_dict={self.input_data: x_batch, self.input_labels: y_batch})

                '''inserted code'''
                scheduler.loss_checker(cost_val)
                '''inserted code'''
