...
weights = {
    ...
    'l1': tf.get_variable(name="w1",
                          initializer=tf.zeros([n_inputs, n_hidden])),
...
# random_variable = l1_matmul['gain']
# random_variable = weights['l1']
...
#         var = tf.Variable(weights['l1'][i][j])
#         var_list.append(tf.Variable(var))
#         obj_list.append(tf.gradients(var,[var]))
...
def sc_matmul(x, w, s_in, upscale=True, gain=None):
    ...
    # tmp_obj_var = max_scale
    rescale_ratio = tf.div(s_in, max_scale+1e-7)
...
with tf.Session() as sess:
    ...
    # sess.run(weights['l1'].initializer)
    ...
    # ''' WG Inserted Start '''
    # obj_function = tf.reduce_mean(tf.abs(random_variable))
    # obj_grads = tf.gradients(obj_function, random_variable)[0]
    # # delta = tf.div(obj_function, obj_grads+1e-7)
    # ''' WG Inserted End '''
    ...
    while True:
        loss_val, random_variable_val, = sess.run([loss, weights['l1'],],
