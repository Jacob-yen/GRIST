...
div1, div2, div3, div4 = None, None, None, None
...
def sc_matmul(x, w, s_in, upscale=True, gain=None):
    ...
    S = tf.reshape(tf.tile(s + 1e-7, [tf.shape(x)[0]]),
    ...
    if (upscale):
        ...
        if div3 is None:
            div3 = gain
        new_scale = tf.div(curr_scale, gain + 1e-7)
    else:
        z_upscaled = z
...
with tf.Session() as sess:
    ...
    scheduler = TensorFlowScheduler(name="SC_DNN_grist")
    '''inserted code'''
    step = 0
    ...
    while True:
        ...
        _, loss_val, random_variable_val, obj_function_val, obj_grads_val, delta_val = sess.run(
            [train_op, loss, random_variable, obj_function, obj_grads, delta], feed_dict={X: batch_x, Y: batch_y})
