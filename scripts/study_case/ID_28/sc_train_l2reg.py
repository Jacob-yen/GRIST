# https://github.com/adamsolomou/SC-DNN/blob/a9169c6b7a0d456c1d2f229913e2d8c042c40aab/src/training/sc_train_l2reg.py
# Success to find NaN!  Iteration: <1> Time cost: <0:00:01.511131>.
...
with tf.Session() as sess:
    tf.train.write_graph(sess.graph_def, '/data/scripts/study_case/pbtxt_files', 'SC_DNN2.pbtxt')
    # Run the initializer
    sess.run(init)
    ...
    scheduler = TensorFlowScheduler(name="SC_DNN_sc_train_l2reg")
    '''inserted code'''
    ...
    #
    # print("Optimal Gain Factor in Layer 1:", l1_matmul['gain'].eval())
    #
    # print("Optimal Gain Factor in Layer 1:", lo_matmul['gain'].eval())
    #
    # W_1, W_out, b_1, b_out = sess.run([weights['l1'], weights['lo'], biases['l1'], biases['lo']])
