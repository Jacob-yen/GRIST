...
def train(sess, batch_size=100, training_epochs=10, display_step=5):
    ...
    scheduler = TensorFlowScheduler(name="ch10_04_06_Pic_10_08_exp")
    '''inserted code'''
    while True:
        avg_cost = 0.
    ...
    for i, yi in enumerate(x_values):
        for j, xi in enumerate(y_values):
            z_mu = np.array([[xi, yi]])
            x_mean = sess.run(x_reconstr_mean, feed_dict={z: z_mu, y: ys[:len(z_mu)]})
            canvas[(nx - i - 1) * 28:(nx - i) * 28, j * 28:(j + 1) * 28] = x_mean[0].reshape(28, 28)

    fig = plt.figure(figsize=(8, 10))
    plt.gray()
    fig.canvas.set_window_title('Рис. 10.8. Примеры рукописных цифр, сэмплированные из разных частей распределения '
...
tf.train.write_graph(sess.graph_def, '/data/scripts/study_case/pbtxt_files', 'ch10_04_06_Pic_10_08.pbtxt')
sess.run(init)
