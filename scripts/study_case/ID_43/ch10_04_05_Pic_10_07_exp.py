...
def show_scatter(sess, data_show, epoch):
    z_mu, y_sample = sess.run((z_mean, x_reconstr_mean),
                              feed_dict={x: data_show[0], y: data_show[1],
...
def train(sess, batch_size=100, training_epochs=10, display_step=5):
    ...
    scheduler = TensorFlowScheduler(name="ch10_04_05_Pic_10_07_exp")
    '''inserted code'''
    while True:
        avg_cost = 0.
...
tf.train.write_graph(sess.graph_def, '/data/scripts/study_case/pbtxt_files', 'ch10_04_05_Pic_10_07.pbtxt')
sess.run(init)
