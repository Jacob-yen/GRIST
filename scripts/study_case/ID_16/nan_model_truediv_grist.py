...
def classifier(images, init_func):
    ...
    input_layer = tf.layers.flatten(images)  # 对图片进行展平操作
    ...
    return input_layer, hidden_layer_1, after_activation_hidden_layer_1, hidden_layer_2, after_activation_hidden_layer_2, output_layer


def unsafe_softmax(logits):  # 用于多分类，分类器最后的输出单元需要softmax进行数值处理
    """Computes softmax in a numerically unstable way."""  # 可能会出现数值溢出
    logits = tf.clip_by_value(logits, tf.reduce_min(logits), 87)
    #MUTATION#
    # obj_var = tf.reduce_sum(tf.exp(logits), axis=1, keepdims=True)
...
def unsafe_cross_entropy(probabilities, labels):  # 描述两个概率分布之间的距离
    ...
    return -tf.reduce_sum(labels * tf.log(tf.abs(probabilities) + 1e-6), axis=1)
...
def main(_):
    ...
    labels = tf.one_hot(integer_labels, 10)  # 使用独热编码对标签编码
    ...
    logits = classifier(x, init_func)  # 分类,得到logits和图片输入张量
    equality = tf.equal(tf.argmax(logits, 1), tf.argmax(y_, 1))  # 比较是否相等，argmax返回最大值的索引号
    ...
    gradient_search = GradientSearcher(name="FuzzForTensorflow_truediv_grist")
    obj_function = tf.reduce_max(logits)
