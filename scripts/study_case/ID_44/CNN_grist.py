...
class CNN(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, X_test=None, y_test=None):
        ...
        gradient_search = GradientSearcher(name="CNN-RelationExtraction.CNN_grist")
        obj_function = tf.reduce_min(tf.abs(self.obj_var))
        obj_grads = tf.gradients(obj_function, self.x)[0]
        batch = data.next_batch()
        batch_xs, batch_ys = batch[0], batch[1]
        max_val, min_val = np.max(batch_xs), np.min(batch_xs)
        gradient_search.build(batch_size=self.batchsize, min_val=min_val, max_val=max_val)
        """insert code"""

        while True:

            """inserted code"""
            monitor_vars = {'loss': self.cross_entropy, 'obj_function': obj_function, 'obj_grad': obj_grads}
            feed_dict = {self.x: batch_xs, self.y_: batch_ys, self.keep_prob: self.dropout}
            batch_xs, scores_rank = gradient_search.update_batch_data(session=self.sess, monitor_var=monitor_vars,
                                                                      feed_dict=feed_dict, input_data=batch_xs, )
            """inserted code"""

            _, loss = self.sess.run([self.train_step, self.cross_entropy], feed_dict=feed_dict)

            """inserted code"""
            new_batch = data.next_batch()
