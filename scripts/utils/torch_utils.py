...
class GradientSearcher(TorchScheduler):
    ...
    @staticmethod
    def iterative_grist(w, dw, pixle_boundary, update_stragety, update_list, clip_list, batch_size, useful_update_list,
                        change, switch_data):
        ...
        if switch_data:
            next_w_ndarray = next_w.detach().numpy()
            delta_val = dw.numpy()
            data_shape = next_w_ndarray.shape[1:]
            pixel_num = CommonUtils.get_pixels_cnt(data_shape)
            next_w_flatten = next_w_ndarray.flatten()
            modified_list = list(np.flatnonzero(abs(delta_val)))
            invalid_pixels = np.argwhere(
                (next_w_flatten > pixle_boundary['max']) | (next_w_flatten < pixle_boundary['min'])).flatten()

            modified_img_idx = [CommonUtils.get_image_idx(pixel_local=pixel, image_shape=data_shape) for pixel in
                                modified_list]
            invalid_img_idx = [CommonUtils.get_image_idx(pixel_local=pixel, image_shape=data_shape) for pixel in
                               invalid_pixels]
    ...
    def update_batch_data(self,monitor_var,input_data,clip=True,):
        # self.change = change if change is not None else self.change
        if self.decay:
        ...
        loss_val,obj_function_val, obj_grads_val = monitor_var['loss'], monitor_var['obj_function'], monitor_var['obj_grad']
        ...
        batch_xs, update_cnt, clip_cnt, useful_update_cnt = GradientSearcher.iterative_grist(input_data, obj_grads_val,
                                                                                             ...
                                                                                             change=decay_change, switch_data=self.switch_data)
        if self.switch_data:
            ...
            scores_rank = None
        if clip:
            batch_xs = torch.clamp(batch_xs, self.pixels['min'], self.pixels['max'])
        ...
        if self.switch_data:
            if self.iter_count % 100 == 0:
                for i, image_idx in enumerate(scores_rank):
                    old_batch_xs[image_idx] = new_batch_xs[i].clone()
                    if input_num == 2:
                        old_batch_ys[image_idx] = new_batch_ys[i].clone()
...
if __name__ == "__main__":
    for i in range(10):
        print(CommonUtils.get_random_learning_rate())
